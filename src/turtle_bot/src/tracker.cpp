...
#define SEUIL_CAMERA 100


//Init pour les slider
...
//Init pour les tests d'états de la caméra
...
void image_callback(const sensor_msgs::ImageConstPtr& theImage, ros::Publisher & pub2, ros::Publisher& pub) {
  ...
  if (isMoving) // Test pour ne pas spammer la caméra et avoir une erreur
   return;

  //ICI, test du service (switch par exemple) afin de choisir le mode
  // si serice_str == "track" => mode track ...
  //parei pour les deux autres modes
  ...
  ROS_INFO("Couleur" );
  Scalar hsv_min(hsv_min_couleur, hsv_min_saturation,  hsv_min_luminosite);//réglage fait par slider
  Scalar hsv_max(hsv_max_couleur, hsv_max_saturation,  hsv_max_luminosite);//réglage fait par slider
  ...
  //Init pour détecter les régions connexes
  std::vector<std::vector<Point> > contours;
  std::vector<Vec4i> hierarchy;

  //Fonction pour récupérer les régions connexes dans contours
  findContours(detection_img, contours, hierarchy, RETR_TREE, CHAIN_APPROX_SIMPLE, Point(0, 0) );

  //Init pour les bounding box
  std::vector<std::vector<Point> > contours_poly( contours.size() );
  std::vector<Rect> boundRect( contours.size() );
  std::vector<Point2f>center( contours.size() );
  std::vector<float>radius( contours.size() );

  //Fonction pour lister les boundings box
  ...
  //Init pour publier des messages PanTilt
...
void theStateCallback(const turtle_bot::Axis & theState){
  ...
  if( ( fabs(theDesireState.pan - theState.pan) < SEUIL_CAMERA ) && ( fabs(theDesireState.tilt - theState.tilt) < SEUIL_CAMERA ) )
    isMoving = false;
...
void creation_affichage (void){
// track bar creation
  namedWindow("windows", 1);
  createTrackbar("Couleur min", "windows", &hsv_min_couleur, 180, NULL);
  createTrackbar("Couleur max", "windows", &hsv_max_couleur, 180, NULL);
  createTrackbar("Saturation min", "windows", &hsv_min_saturation, 256, NULL);
  createTrackbar("Saturation max", "windows", &hsv_max_saturation, 256, NULL);
  createTrackbar("Luminosite min", "windows", &hsv_min_luminosite, 256, NULL);
  createTrackbar("Luminosite max", "windows", &hsv_max_luminosite, 256, NULL);
  // end
}

//1 => recherche d'un Turtlebot (mode search)
//2 si Turtle => suivi du robot qui engendre le moins de mouvements pour la caméra => déplacement relatif (trier par rapport à la liste "centre")
//2 bis => si liste vide, passe en mode scan => bouge la caméra jusqu'à trouver la caméra => si robot "vu" re-passe en mode 
