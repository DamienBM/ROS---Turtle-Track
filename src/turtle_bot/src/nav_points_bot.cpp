...
bool moveTo(const geometry_msgs::Pose2D& target_pose, geometry_msgs::Twist& cmd_vel){
  ...
  if (dist < (tol_lin/100.0)){
    //you need to turn the robot to have the correct orientation
    cmd_vel.linear.x = 0;
    
    if (fabs(target_pose.theta) > (tol_ang/100.0)){
    // a little bit of rotation
      cmd_vel.angular.z = copysign((vit_ang/100.0),target_pose.theta);
    }
    else{
      cmd_vel.angular.z = 0;
    ...
    if (target_pose.x == 0){
      cmd_vel.angular.z = copysign((vit_ang/100.0),target_pose.y);}
    else{
	...
	angle = angle - M_PI;
      ...
      if (fabs(angle) > (tol_ang/100.0)){
	cmd_vel.angular.z = copysign((vit_ang/100.0), angle);
	cmd_vel.linear.x = 0;
      }
      else{
	cmd_vel.linear.x = (vit_lin/100.0);  
	cmd_vel.angular.z = 0; 
      }    
    }
  }
...
int main(int argc, char  **argv){
  ...
  */
  ...
  cv::namedWindow("Speed params", 1);
  cv::createTrackbar("vitesse lineaire", "Speed params", &vit_lin, 100, NULL);
  cv::createTrackbar("vitesse angulaire", "Speed params", &vit_ang, 100, NULL);
  cv::createTrackbar("tolerance lineaire", "Speed params", &tol_lin, 100, NULL);
  cv::createTrackbar("tolerance angulaire", "Speed params", &tol_ang, 100, NULL);
